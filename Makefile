...
OPENCL:
	...
	cd opencl/backprop;			make;	cp backprop.out     $(OPENCL_BIN_DIR)
	cd opencl/bfs;				make;	cp bfs.out		$(OPENCL_BIN_DIR)
	cd opencl/b+tree;			make;	cp b+tree.out	$(OPENCL_BIN_DIR)
	cd opencl/cfd;				make;	cp euler3d.out	$(OPENCL_BIN_DIR)
	cd opencl/hotspot;			make;	cp hotspot.out	$(OPENCL_BIN_DIR)
	cd opencl/kmeans;			make;	cp kmeans.out	$(OPENCL_BIN_DIR)
	cd opencl/lavaMD;			make;	cp lavaMD.out	$(OPENCL_BIN_DIR)
	cd opencl/leukocyte;			make;	cp leukocyte.out	$(OPENCL_BIN_DIR)
	cd opencl/lud/ocl;			make;	cp lud.out		$(OPENCL_BIN_DIR)
	cd opencl/myocyte;			make;	cp myocyte.out	$(OPENCL_BIN_DIR)
	cd opencl/nw;				make;	cp nw.out		$(OPENCL_BIN_DIR)
	cd opencl/srad;				make;	cp srad.out		$(OPENCL_BIN_DIR)
	cd opencl/streamcluster;		make;	cp streamcluster.out	$(OPENCL_BIN_DIR)
	...
	cd opencl/gaussian;			make;	cp gaussian.out	$(OPENCL_BIN_DIR)
	cd opencl/nn;				make;	cp nn.out	$(OPENCL_BIN_DIR)
	cd opencl/heartwall;		        make;	cp heartwall.out	$(OPENCL_BIN_DIR)
	cd opencl/hybridsort;              	make;   cp hybridsort.out $(CUDA_BIN_DIR)
	cd opencl/dwt2d;                   	make;   cp dwt2d.out  $(CUDA_BIN_DIR)
